...
window.addEventListener("DOMContentLoaded",callback,false);else{EVENTS.bind(window,"load",callback);}};EVENTS.onMediaReady=function(callback){EVENTS.bind(window,"MediaContentLoaded",callback);};EVENTS.onSMILReady=function(callback){EVENTS.bind(window,"SMILContentLoaded",callback);};window.EVENTS=EVENTS;}(this,document);!function(window,document,undefined){var gSupported=true;var gNative=false;function qwerySelector(cssQuery,context){}
...
return results;}};if(document.querySelector){qwerySelector=function(cssQuery,context){context=context||document;return context.querySelector(cssQuery);};}
else{qwerySelector=function(cssQuery,context){var results=qwerySelectorAll(cssQuery,context);if(results&&results.length)
...
return results;}
window.QWERY={select:qwerySelector,selectAll:qwerySelectorAll,selectTimeContainers:qweryTimeContainers,selectExtTimesheets:qweryExtTimesheets,supported:gSupported,nativeSelector:gNative};}(this,document);if(!Array.indexOf)Array.prototype.indexOf=function(obj){for(var i=0;i<this.length;i++){if(this[i]==obj){return i;}}
return-1;};if(!Date.now)Date.now=function(){var timestamp=new Date();return timestamp.getTime();};!function(window,document,undefined){var DEBUG=true;function consoleLog(message){if(DEBUG&&(typeof(console)=="object"))
console.log(message);}
function consoleWarn(message){if(typeof(console)=="object")
console.warn(message);}
var TIMERATE=40;if(window.mejs)
...
EVENTS.bind(document,"MediaElementLoaded",CountMediaElements);for(var i=0;i<allAudioElements.length;i++)
parseMediaElement(allAudioElements[i]);for(i=0;i<allVideoElements.length;i++)
...
function parseAllTimeContainers(){TIMECONTAINERS=[];var timesheets=QWERY.selectExtTimesheets();var tsLength=timesheets.length;var tsParsed=0;function CountTimesheets(){tsParsed++;if(tsParsed>tsLength){EVENTS.unbind(document,"SMILTimesheetLoaded",CountTimesheets);EVENTS.trigger(window,"SMILContentLoaded");}}
EVENTS.bind(document,"SMILTimesheetLoaded",CountTimesheets);var xhr;for(var i=0;i<tsLength;i++){if(window.ActiveXObject){xhr=new ActiveXObject("Microsoft.XMLHTTP");xhr.open("GET",timesheets[i].href,true);xhr.onreadystatechange=function(){if(xhr.readyState==4){var xmlDoc=new ActiveXObject("Microsoft.XMLDOM");xmlDoc.loadXML(xhr.responseText);var tsNodes=xmlDoc.getElementsByTagName("timesheet");if(tsNodes&&tsNodes.length)
...
var TimesheetNS=nsResolver("smil");timesheets=document.getElementsByTagNameNS(TimesheetNS,"timesheet");if(!timesheets.length)
timesheets=document.getElementsByTagName("timesheet");for(i=0;i<timesheets.length;i++)
parseTimesheetNode(timesheets[i]);if(docElt.getAttribute("xmlns")){var containers=document.evaluate("//*[@smil:timeContainer]",document,nsResolver,XPathResult.ORDERED_NODE_ITERATOR_TYPE,null);var thisContainer=containers.iterateNext();try{while(thisContainer){parseTimeContainerNode(thisContainer);thisContainer=containers.iterateNext();}}catch(e){}}}
var allTimeContainers=QWERY.selectTimeContainers();for(i=0;i<allTimeContainers.length;i++)
parseTimeContainerNode(allTimeContainers[i]);EVENTS.trigger(document,"SMILTimesheetLoaded");}
...
function smilExternalTimer(mediaPlayerNode){var self=this;var currentTime=NaN;this.onTimeUpdate=null;var autoplay=true;var mediaPlayerAPI=mediaPlayerNode;if(mediaPlayerNode.mediaAPI){mediaPlayerAPI=mediaPlayerNode.mediaAPI;consoleLog("MediaElement interface found.");}
...
mediaPlayerAPI.addEventListener("canplay",setThisTimeErr,false);}};this.Play=function(){consoleLog("starting continuous timeContainer");if(mediaPlayerAPI.addEventListener)
mediaPlayerAPI.addEventListener("timeupdate",self.onTimeUpdate,false);if(autoplay){consoleLog("play!");consoleLog(mediaPlayerAPI);mediaPlayerAPI.play();}};this.Pause=function(){if(mediaPlayerAPI.pause)
mediaPlayerAPI.pause();};this.Stop=function(){if(mediaPlayerAPI.removeEventListener)
mediaPlayerAPI.removeEventListener("timeupdate",self.onTimeUpdate,false);if(autoplay){consoleLog("pause!");mediaPlayerAPI.pause();}};}
...
target.className=active?target._smilclass_active:target._smilclass_idle;};switch(timeAction){case"display":return setTargetState_display;break;case"visibility":return setTargetState_visibility;break;case"style":return setTargetState_style;break;case"intrinsic":if(OLDIE)
return setTargetState_display;else
return setTargetState_intrinsic;default:if(/^class:/.test(timeAction))
return setTargetState_class;else
return setTargetState_display;break;}
return null;};smilTimeItem.prototype.addEventListener=function(events,callback){for(var i=0;i<events.length;i++){var evt=events[i];if(evt.target)
EVENTS.bind(evt.target,evt.event,callback);}};smilTimeItem.prototype.removeEventListener=function(events,callback){for(var i=0;i<events.length;i++){var evt=events[i];if(evt.target)
EVENTS.unbind(evt.target,evt.event,callback);}};smilTimeItem.prototype.dispatchEvent=function(eventType){var func=this["on"+eventType];EVENTS.trigger(this.parentTarget,eventType);if(func)
func.call(this.parentTarget);};function smilTimeItem(domNode,parentNode,targetNode){var self=this;this.parseTime=smilTimeItem.prototype.parseTime;this.parseEvents=smilTimeItem.prototype.parseEvents;this.parseAttribute=smilTimeItem.prototype.parseAttribute;this.parentNode=parentNode;this.previousSibling=null;this.nextSibling=null;this.timeNodes=null;this.getNode=function(){return domNode;};this.target=targetNode||domNode;if(/^(smil:){0,1}(par|seq|excl)$/i.test(this.target.nodeName))
...
function onendListener(){consoleLog("onendListener");self.time_in=isNaN(self.begin)?Infinity:self.begin;self.time_out=self.parentNode.getCurrentTime();if(self.parentNode.timeContainer=="seq")
self.parentNode.selectIndex(self.parentNode.currentIndex+1);else
self.parentNode.currentIndex=-1;self.hide();}
...
self.setTargetState(state);self.addEventListener(beginEvents,onbeginListener);self.removeEventListener(endEvents,onendListener);};this.kill=onendListener;if(targetNode&&(targetNode!=domNode)){if(!targetNode.extTiming)
targetNode.extTiming=[];targetNode.extTiming.push(this);consoleLog("extTiming: "+targetNode.nodeName);}else if(this.target){domNode.timing=this;}}
...
node=new smilTimeElement(segment,this);if(node.parseAttribute("syncMaster"))
syncMasterNode=target;if(node.timeAction!="none")
timeNodes.push(node);else
delete(node);}}}
for(i=0;i<timeNodes.length;i++){segment=timeNodes[i];if(i>0)
segment.previousSibling=timeNodes[i-1];if(i<timeNodes.length-1)
segment.nextSibling=timeNodes[i+1];segment.parentNode=this;}
if(this.dur==undefined){if(!isNaN(this.end-this.begin))
this.dur=this.end-this.begin;else
this.dur=Infinity;}
return{timeNodes:timeNodes,syncMasterNode:syncMasterNode};};smilTimeContainer_generic.prototype.computeTimeNodes=function(){};smilTimeContainer_generic.prototype.getMediaSync=function(syncMasterNode){var mediaSyncSelector=this.parseAttribute("mediaSync");return QWERY.select(mediaSyncSelector)||syncMasterNode;};smilTimeContainer_generic.prototype.currentIndex=-1;smilTimeContainer_generic.prototype.selectIndex=function(index){if(this.repeatCount==Infinity)
index=index%this.timeNodes.length;if((index>=0)&&(index<this.timeNodes.length)&&(index!=this.currentIndex)){consoleLog("current index: "+this.currentIndex);var time=this.timeNodes[index].time_in;if(!isNaN(time)&&(time<Infinity)){consoleLog("hashchange: set time to "+time);if(this.mediaSyncNode){this.setCurrentTime(time+0.1);this.onTimeUpdate();return;}else
this.setCurrentTime(time);}
this.currentIndex=index;this.timeNodes[index].show();for(var i=0;i<index;i++)
this.timeNodes[i].hide();for(i=index+1;i<this.timeNodes.length;i++)
this.timeNodes[i].reset();this.dispatchEvent("change");consoleLog("new index: "+this.currentIndex);}};smilTimeContainer_generic.prototype.selectItem=function(item){var index=this.timeNodes.indexOf(item);this.selectIndex(index);};function smilTimeContainer_generic(timeContainerNode,parentNode,timerate){this.parseTimeNodes=smilTimeContainer_generic.prototype.parseTimeNodes;this.getMediaSync=smilTimeContainer_generic.prototype.getMediaSync;var self=this;this.repeatCount=this.parseAttribute("repeatCount",1);this.repeatDur=this.parseAttribute("repeatDur",NaN);consoleLog("  initializing: "+this.timeContainer+" ("+this.getNode().nodeName+")");var result=this.parseTimeNodes();this.timeNodes=result.timeNodes;this.computeTimeNodes();if(true){consoleLog("  time container: "+this.timeContainer+" ("+this.getNode().nodeName+")");for(var i=0;i<this.timeNodes.length;i++)
consoleLog("    timeNodes["+i+"]: "+this.timeNodes[i].time_in+" => "+this.timeNodes[i].time_out);}
this.mediaSyncNode=this.getMediaSync(result.syncMasterNode);var timer=null;this.mediaSyncAPI=this.mediaSyncNode;if(this.mediaSyncNode){timer=new smilExternalTimer(this.mediaSyncNode);if(this.mediaSyncNode.mediaAPI)
this.mediaSyncAPI=this.mediaSyncNode.mediaAPI;}else{timer=new smilInternalTimer(timerate);}
...
function onendListener(){consoleLog("onendListener");self.time_in=isNaN(self.begin)?Infinity:self.begin;self.time_out=self.parentNode.getCurrentTime();self.parentNode.currentIndex=-1;if(self.parentNode){if(self.parentNode.timeContainer=="seq")
self.parentNode.selectIndex(self.parentNode.currentIndex+1);else
self.parentNode.currentIndex=-1;self.hide();}}
var state="";this.isActive=function(){return(state=="active");};this.show=function(){if(state=="active")return;state="active";self.Play();self.setTargetState(state);self.dispatchEvent("begin");self.addEventListener(endEvents,onendListener);self.removeEventListener(beginEvents,onbeginListener);this.currentIndex=-1;};this.hide=function(){if(state=="done")return;state="done";self.Stop();self.setTargetState(state);for(var i=0;i<self.timeNodes.length;i++){self.timeNodes[i].hide();if(self.timeNodes[i].fill!="hold")
self.timeNodes[i].setTargetState("done");}
self.dispatchEvent("end");self.addEventListener(beginEvents,onbeginListener);self.removeEventListener(endEvents,onendListener);};this.reset=function(){if(state=="idle")return;state="idle";self.Stop();self.setTargetState(state);for(var i=0;i<self.timeNodes.length;i++)
self.timeNodes[i].reset();self.addEventListener(beginEvents,onbeginListener);self.removeEventListener(endEvents,onendListener);this.currentIndex=-1;};this.kill=onendListener;TIMECONTAINERS.push(this);}
smilTimeContainer_par.prototype.computeTimeNodes=function(){for(var i=0;i<this.timeNodes.length;i++){var segment=this.timeNodes[i];segment.reset();if(segment.begin!=undefined)
segment.time_in=isNaN(segment.begin)?Infinity:segment.begin;else
segment.time_in=0;if(segment.dur!=undefined)
segment.time_out=segment.time_in+segment.dur;else if(segment.end!=undefined)
segment.time_out=isNaN(segment.end)?Infinity:segment.end;else
segment.time_out=this.dur;}};smilTimeContainer_par.prototype.onTimeUpdate=function(){var time=this.getCurrentTime();if(this.repeatCount>=Infinity)
time=time%this.dur;for(var i=0;i<this.timeNodes.length;i++){if(time<this.timeNodes[i].time_in)
this.timeNodes[i].reset();else if(time>=this.timeNodes[i].time_out)
this.timeNodes[i].hide();else
this.timeNodes[i].show();}};function smilTimeContainer_par(domNode,parentNode,timerate){this.computeTimeNodes=smilTimeContainer_par.prototype.computeTimeNodes;this.onTimeUpdate=smilTimeContainer_par.prototype.onTimeUpdate;smilTimeContainer_generic.call(this,domNode,timerate);this.currentIndex=-1;this.selectIndex=function(index){};this.selectItem=function(item){if(!isNaN(item.time_in))
this.setCurrentTime(item.time_in);item.show();};}
smilTimeContainer_excl.prototype.computeTimeNodes=function(){var segment=null;for(var i=0;i<this.timeNodes.length;i++){segment=this.timeNodes[i];segment.reset();if(segment.begin!=undefined)
segment.time_in=isNaN(segment.begin)?Infinity:segment.begin;else
segment.time_in=Infinity;if(segment.end!=undefined)
segment.time_out=isNaN(segment.end)?Infinity:segment.end;else if((i<this.timeNodes.length-1)&&!isNaN(this.timeNodes[i+1].begin))
segment.time_out=this.timeNodes[i+1].begin;else if(!isNaN(segment.dur))
segment.time_out=segment.time_in+segment.dur;else
segment.time_out=this.dur;}
if(!segment)
return;if(!this.timeNodes[0].time_in)
this.timeNodes[0].show();if(this.dur==undefined)
this.dur=segment.time_out-this.timeNodes[0].time_in;};smilTimeContainer_excl.prototype.onTimeUpdate=function(){var time=this.getCurrentTime();if(time>this.clipEnd){this.kill();return;}
...
return;}
var index=-1;var active=false;for(var i=0;i<this.timeNodes.length;i++){var segment=this.timeNodes[i];var withinBounds=(time>=segment.time_in)&&(time<segment.time_out);if(time<segment.time_in)
segment.reset();else if(time>=segment.time_out)
segment.hide();else if(withinBounds){if(active){segment.reset();}else{active=true;segment.show();index=i;}}}
...
segment.time_in=segment.begin;else if((i>0)&&(this.timeNodes[i-1].time_out<Infinity))
segment.time_in=this.timeNodes[i-1].time_out;else
segment.time_in=0;if(!isNaN(segment.dur))
segment.time_out=segment.time_in+segment.dur;else if(i==this.timeNodes.length-1)
segment.time_out=this.dur;else
segment.time_out=Infinity;}
if(!segment)
return;if((this.dur==undefined)||(this.dur>=Infinity))
this.dur=segment.time_out;if(this.end==undefined)
this.end=segment.time_out+this.begin;};smilTimeContainer_seq.prototype.onTimeUpdate=function(){var time=this.getCurrentTime();var withinBounds,outOfBounds,segment;if(this.repeatCount>=Infinity)
time=time%this.dur;if(this.currentIndex>=0){segment=this.timeNodes[this.currentIndex];outOfBounds=(time<segment.time_in)||(time>=segment.time_out);withinBounds=(time>=segment.time_in)&&(time<segment.time_out);if(withinBounds)
return;else
...
var index=-1;var active=false;for(var i=0;i<this.timeNodes.length;i++){segment=this.timeNodes[i];withinBounds=(time>=segment.time_in)&&(time<segment.time_out);if(time<segment.time_in)
segment.reset();else if(time>=segment.time_out)
segment.hide();else if(withinBounds){if(active){segment.reset();}else{active=true;segment.show();index=i;}}}
if(index>=0){this.currentIndex=index;}
...
if(this.timeContainer){consoleLog("  "+this.timeContainer+" ("+domNode.nodeName+") properly initialized.");}}
document.createTimeContainer=function(domNode,parentNode,targetNode,timerate){return new smilTimeElement(domNode,parentNode,targetNode,timerate);};document.getTimeNodesByTarget=function(node){var timeNodes=[];if(!node)return timeNodes;if(node.timing)
timeNodes.push(node.timing);if(node.extTiming){for(var i=0;i<node.extTiming.length;i++)
timeNodes.push(node.extTiming[i]);}
timeNodes.item=function(index){return timeNodes[index];};return timeNodes;};document.getTimeContainersByTarget=function(node){var contNodes=[];var timeNodes=document.getTimeNodesByTarget(node);for(var i=0;i<timeNodes.length;i++){if(timeNodes[i].timeContainer)
contNodes.push(timeNodes[i]);}
contNodes.item=function(index){return contNodes[index];};return contNodes;};document.getTimeContainersByTagName=function(tagName){var contNodes=[];tagName=tagName.toLowerCase();if((/^(par|seq|excl)$/).test(tagName)){for(var i=0;i<TIMECONTAINERS.length;i++){if(TIMECONTAINERS[i].timeContainer.toLowerCase()==tagName)
contNodes.push(TIMECONTAINERS[i]);}}else if(tagName=="*"){contNodes=TIMECONTAINERS;}
